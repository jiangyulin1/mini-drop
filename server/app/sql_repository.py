...
class SqlRepository:
    ...
    def create_runtime_wakeup(
        ...
        with self._write_session() as session:
            existing = session.query(RuntimeWakeupModel).filter(
                RuntimeWakeupModel.dedupe_key == dedupe_key,
                RuntimeWakeupModel.status == "PENDING",
            ).first()
            if existing is not None:
                existing.to_evidence_watermark = max(
                    existing.to_evidence_watermark, int(to_evidence_watermark or 0),
                )
                existing.source_refs = sorted(set(
                    (existing.source_refs or []) + list(source_refs or []),
                ))
                existing.updated_at = now
                return existing.to_dict()
